      ...
      <main className="min-h-screen justify-between p-24 bg-base">
        ...
        <ul>
          {teams && teams.map((team) => {
            return (
              <li key={team._id}><Link href={`/teams/${team._id}`}>{team.name}</Link></li>
            )
          })}
        </ul>
        <div className="drawer drawer-end">
          ...
          <div className="drawer-content">
            <label htmlFor="my-drawer-4" className="drawer-button btn btn-primary">Enter match results</label>
          </div> 
            ...
            <div className="menu p-4 w-80 min-h-full bg-base-200 text-base-content">
              {pendingMatches && pendingMatches.map((match) => {
                ...
                return (
                    ...
                    <form action={formAction} className="p-2 shadow menu dropdown-content z-[1] bg-base-100 rounded-box w-52">
                      <label>
                        {match.participants[0].name}
                        <input type="radio" name="winner" id="first" value={match.participants[0].id} className="radio" required />
                      </label>
                      <label>
                        {match.participants[1].name}
                        <input type="radio" name="winner" id="second" value={match.participants[1].id} className="radio" />
                      </label>
                      <label className="input input-bordered flex items-center gap-2 w-full max-w-xs mx-auto my-2">
                      Winner Score:
                      <input name='winnerScore' id='winnerScore' type='text' placeholder="Score" required/>
                      </label>
                      <label className="input input-bordered flex items-center gap-2 w-full max-w-xs mx-auto my-2">
                      Loser Score:
                      <input name='loserScore' id='loserScore' type='text' placeholder="Score"  required/>
                      </label>
                      <input type="hidden" id="matchId" name="matchId" value={match.id} />
                      <input type="hidden" id="team1" name="team1" value={match.participants[0].id} />
                      <input type="hidden" id="team2" name="team2" value={match.participants[1].id} />
